...
module.exports = class ApiClient {
  ...
  static async selectApiPeer (peers) {
    for (const peer of peers) {
      ...
      if (config && config.plugins && config.plugins['@phantomcores/core-api'] &&
      config.plugins['@phantomcores/core-api'].enabled) {
        return {
          ...peer,
          port: config.plugins['@phantomcores/core-api'].port
        }
      }
    }
